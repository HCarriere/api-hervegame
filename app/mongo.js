...
function initMongo(){
	...
	logMsg("api:mongo:():initMongo:OK:(mongo initialized)");
}
   ...
   /*var conn = mongoose.createConnection(conf.database.name, function(err) {
    ...
    if(!conn){
        ...
        conn = mongoose.createConnection(address, options, function(err) {
           if(!err){
               callback(null);
			   logMsg("api:mongo:():openConnection:OK:(mongo connected to "+address+" )");
           }else{
			   callback(err);
			   logMsg("api:mongo:():openConnection:ERR:(mongo is unable to connect to "+address+" )");
			   return;
		   }
        })
        ...
        if(conn){
            ...
            objectFromModel.save(function (err) {
                if (err) { 
                    ...
                    logMsg("api:mongo:():addObject:OK:(Object added to "+schema.collection+")");
    ...
    openConnection(function(coErr){
        if(conn){
            ...
            model.find(jsonRequest, function (err, result) {
                ...
                logMsg("api:mongo:():findObject:OK:("+result.length+" object found in "+schema.collection+")");
                closeConnection();
    ...
    openConnection(function(coErr){
        if(conn){
            ...
            model.find(jsonRequest, function (err, result) {
                ...
                logMsg("api:mongo:():findObjectWithOptions:OK:("+result.length+" object found in "+schema.collection+")");
                closeConnection();
                callback(null, result);
				return;
            })
    ...
    openConnection(function(coErr){
        if(conn){
            ...
            model.findOne(jsonRequest, function (err, result) {
                ...
                logMsg("api:mongo:():findOne:OK:(object found in "+schema.collection+")");
                closeConnection()
                callback(null, result);
				return;
            });
    ...
    openConnection(function(coErr){
        if(conn){
			...
			model.findById(id, function (err, result) {
				...
				logMsg("api:mongo:():findById:OK:(object found in "+schema.collection+" with id "+id+")");
				closeConnection()
				callback(null, result);
				return;
			});
    ...
    openConnection(function(coErr){
        if(conn){
            ...
            model.findById(id).remove(function(err,result){
                ...
                logMsg("api:mongo:():removeById:OK:("+result.length+" objects removed with id "+id+")");
                closeConnection()
                callback(null, result);
				return;
            });
    ...
    openConnection(function(coErr){
        if(conn){
            ...
            model.update(condition, update, option, function(err){
                ...
                logMsg(`api:mongo:():updateObject:OK:(update ok : ${JSON.stringify(condition)} --> ${JSON.stringify(update)})`);
                closeConnection();
    ...
    openConnection(function(coErr){
        if(conn){
            ...
            model.remove(condition, function (err) {
                ...
                logMsg(`api:mongo:():updateObject:OK:(remove ok (${JSON.stringify(condition)}))`);
                closeConnection();
        ...
        if(conn){
            ...
            model.count(condition , function(err, count){
                if(!err && count){
                    closeConnection();
                    callback(null, count);
                    logMsg(`api:mongo:():updateObject:OK:( ${count} object have ${JSON.stringify(condition)})`);
					return;
                }else {
                    closeConnection();
                    callback(err, null);
